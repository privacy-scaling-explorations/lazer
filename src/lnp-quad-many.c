 ...
 */
void
lnp_quad_many_prove (uint8_t hash[32], polyvec_t tB, poly_t c, polyvec_t z1,
                     ...
                     const uint8_t seed[32], const abdlop_params_t params)
{
  ...
  DEBUG_PRINTF (DEBUG_PRINT_FUNCTION_ENTRY, "%s", "lnp_quad_many_prove");
  STOPWATCH_START (stopwatch_lnp_quad_many_prove, "lnp_quad_many_prove begin");
  ...
  _schwartz_zippel_poly (R2, r1, NULL, hash, R2i, r1i, NULL, N, params);

  /* seed can be passed directly to quad sub-protocol since it is not used. */
  lnp_quad_prove (hash, tB, c, z1, z21, h, s1, m, s2, tA2, A1, A2prime, Bprime,
                  R2, r1, seed, params);

  spolymat_free (R2);
  spolyvec_free (r1);

  STOPWATCH_STOP (stopwatch_lnp_quad_many_prove);
  DEBUG_PRINTF (DEBUG_PRINT_FUNCTION_RETURN, "%s", "lnp_quad_many_prove end");
}
